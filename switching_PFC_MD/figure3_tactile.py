...
data_path = os.path.join(data_root, 'outcome_tactile_EI_ver2_6.7/')
...
def switching_sem_combine(task,type):
    ...
    idx_list = [0,5,8,9, 10, 13,14,15,18,19]#0,5,8,9, 10,13,14,15,18,19,  12,7, 17,1
    models_0 = idx_list#
    models_1 = idx_list#
    ...
    # plt.title(file_name+';  '+str(idx_list ), fontsize=8)
    plt.legend(fontsize=8)
    # fig.savefig(figure_path + file_name +'switching.pdf')
    plt.show()

switching_sem_combine(task='tactile',type='type1')
...
def plot_diff_uncert_box(type):
    ...
    # models_0 = [0,5,8,9, 10, 13,14,15,18,19]
    # models_1 = [0,5,8,9, 10, 13,14,15,18,19]
    #
    #
    #
    #
...
def plot_diff_uncert_box_conA(type):
    ...
    # model_lsit = range(10,20)#[1,2,4,5,6,   8,10,13,0,16]
    #
    ...
    # models_3 = range(2,12)#[0,1,2,3,4, 5,6,7,17,16]#range(20)#[1,2,4,5,6,   8,10,13,0,16]
    #
    #
    #
    #
...
def switching_sem_combine_different_type(task,p_out_uncert):
    ...
    # P_correct_list = []
    #
    # models_0 = [13,14,17,19,20, 21,24,25,26,27]#12,13,14,17,19,20,21,24,25,26,27
    # models_1 = [0,4,10,12, 14,  15,16,17,18,19]#0,4,6,8,9,10,12,14                2,5,7
    #
    # P_choice_list_0 = []
    # P_correct_list_0 = []
    #
    ...
    #     pre_switch = 20
    #     post_switch = 31
    #     P_choice_conA_0,P_correct_conA_0,_ = get_log(model_idx=model_idx, context='conA',p_out_uncert=1.0)
    #     P_choice_conB_0,P_correct_conB_0,_ = get_log(model_idx=model_idx, context='conB',p_out_uncert=1.0)
    #
    #     P_choice_0 = P_choice_conA_0[-pre_switch:]+P_choice_conB_0[0:post_switch]
    #     P_correct_0 = P_correct_conA_0[-pre_switch:]+P_correct_conB_0[0:post_switch]
    #
    ...
    #     pre_switch = 20
    #     post_switch = 16
    #     P_choice_conA_1,P_correct_conA_1,_ = get_log(model_idx=model_idx, context='conA',p_out_uncert=1.0)
    #     P_choice_conB_1,P_correct_conB_1,_ = get_log(model_idx=model_idx, context='conB',p_out_uncert=1.0)
    #
    #     P_choice_1 = P_choice_conA_1[-pre_switch:]+P_choice_conB_1[0:post_switch]
    #     P_correct_1 = P_correct_conA_1[-pre_switch:]+P_correct_conB_1[0:post_switch]
    #
    ...
    # P_correct_numpy_1 = np.array(P_correct_list_1)
    #
    # np.save(data_path + 'P_choice_numpy_type0.npy', P_choice_numpy_0)
    # np.save(data_path + 'P_choice_numpy_type1.npy', P_choice_numpy_1)
    # np.save(data_path + 'P_correct_numpy_type0.npy', P_correct_numpy_0)
    # np.save(data_path + 'P_correct_numpy_type1.npy', P_correct_numpy_1)

    P_choice_numpy_0 = np.load(data_path + 'P_choice_numpy_type0.npy')
    P_choice_numpy_1 = np.load(data_path + 'P_choice_numpy_type1.npy')
    P_correct_numpy_0 = np.load(data_path + 'P_correct_numpy_type0.npy')
    P_correct_numpy_1 = np.load(data_path + 'P_correct_numpy_type1.npy')
    ...
    # Plot means and SEM shaded area
    ax.plot(x_0, P_correct_mean_0, c=colors[0], label='type0')
    ax.fill_between(x_0, P_correct_mean_0 - P_correct_sem_0, P_correct_mean_0 + P_correct_sem_0,
                    color=colors[0], alpha=0.3)

    # ax.plot(x_0, P_choice_mean_0, c='gray', label='type0')
    # ax.fill_between(x_0, P_choice_mean_0 - P_choice_sem_0, P_choice_mean_0 + P_choice_sem_0,
    ...
    # Plot means and SEM shaded area
    ax.plot(x_1, P_correct_mean_1, c=colors[1], label='type1')
    ax.fill_between(x_1, P_correct_mean_1 - P_correct_sem_1, P_correct_mean_1 + P_correct_sem_1,
                    color=colors[1], alpha=0.3)

    # ax.plot(x_1, P_choice_mean_1, c='gray', label='type1')
    # ax.fill_between(x_1, P_choice_mean_1 - P_choice_sem_1, P_choice_mean_1 + P_choice_sem_1,
    #                 color='gray', alpha=0.4)
    ...
    # file_name='combine_'+task+'_'+hp['type']+'_bs'+str(hp['batch_size'])+'_uncert'+str(p_out_uncert)
    # plt.title(file_name, fontsize=8)
    # plt.legend(fontsize=8)
    # fig.savefig(figure_path + file_name +'_different_type.pdf')
    plt.show()

switching_sem_combine_different_type(task='tactile',p_out_uncert=1.0)
