...
figure_path = os.path.join(fig_path, 'decode_lib/')
tools.mkdir_p(figure_path)
data_path = os.path.join(hp['root_path'], 'Datas','decode_lib/')
...
def plot_select_decode(model_idx, idx):
    ...
    epoch = decode_lib.get_epoch(model_dir=model_dir, rule_name='retro', hp=hp)
    ...
    #     hp['seed'] = seed
    #
    #     accuracy_ppc,accuracy_pfc = decode_lib.decode_up_low_both_new(figure_path, model_name, model_dir, idx, hp, sigma_x=0.3)
    #
...
for model_idx in np.array([23]):  # 3,13,23
...
def plot_decode_generalize(model_idx,idx,generalize):
    get_model_hp()
    ...
    #         accuracy_ppc,accuracy_pfc=decode_lib.decode_up_low_both_generalize_retro2prosp(figure_path, model_name, model_dir, idx, hp)
    #
    #     elif generalize=='prosp2retro':
    #         accuracy_ppc,accuracy_pfc=decode_lib.decode_up_low_both_generalize_prosp2retro(figure_path, model_name, model_dir, idx, hp)
    #
    #     accuracy_ppc_list.append(accuracy_ppc)
    #     accuracy_pfc_list.append(accuracy_pfc)
    #
    # accuracy_ppc_list=np.array(accuracy_ppc_list)
    # accuracy_pfc_list = np.array(accuracy_pfc_list)
    # np.save(data_path+'accuracy_ppc_list'+file_name+'_'+generalize+'.npy',accuracy_ppc_list)
    # np.save(data_path+'accuracy_pfc_list'+file_name+'_'+generalize+'.npy',accuracy_pfc_list)
    ...
    accuracy_ppc_list = np.load(data_path+'accuracy_ppc_list'+file_name+'_'+generalize+'.npy')
    accuracy_pfc_list = np.load(data_path + 'accuracy_pfc_list' + file_name + '_' + generalize + '.npy')
    print('accuracy_pfc_list',accuracy_pfc_list.shape)
    ...
    fig = plt.figure(figsize=(2.7, 2.7))
    ax = fig.add_axes([0.25, 0.2, 0.7, 0.6])
    plt.title(file_name +': '+generalize, fontsize=9)
    start = stim1_off + 1
    ...
    plt.legend(fontsize=8)
    plt.savefig(fig_path1 + file_name + '_' + str(idx) + '_' + str(hp['sigma_x']) +'_'+generalize+ '.pdf')
    plt.show()
#

for model_idx in np.array([23]):  # 6, 65,71
    for idx in np.array([15]):  # 23
        plot_decode_generalize(model_idx,idx,generalize='retro2prosp')
        plot_decode_generalize(model_idx, idx, generalize='prosp2retro')
